...
const to_weeklyList_ToggleButton = document.getElementById(
  "to_weeklyList_ToggleButton"
);
const btn_submit_edit = document.getElementById("btn_submit_edit");
const btn_delete_shoppinglist = document.getElementById(
  "btn_delete_shoppinglist"
);
const btn_delete_product = document.getElementById("btn_delete_product");
const btn_trigger_weekly_list = document.getElementById(
  "btn_trigger_weekly_list"
);
...
function load_default_data() {
  data.forEach((default_product) => {
    let product_exists = false;
    for (let i = 0; i < products.length; i++) {
      if (default_product.product_name === products[i].product_name) {
        product_exists = true;
        break;
      }
    }
...
function add_new_product() {
  ...
  if (new_product_name.length > 0) {
    ...
    if (product_exists === false) {
      const prod = new Product(uniqueID_Generator(), new_product_name, 0.0);
      prod.is_on_list = true;
      prod.is_open = true;
      products.unshift(prod);
      shoppinglist.unshift(prod);
...
btn_delete_shoppinglist.addEventListener("click", () => {
  const decision = window.confirm(
    "Sollen alle Produkte auf der Einkaufsliste entfernt werden?"
  );
...
btn_delete_product.addEventListener("click", () => {
  const decision = window.confirm(
    `Soll das Produkt "${current_product.product_name}" für immer gelöscht werden?`
  );
...
btn_save_weekly_list.addEventListener("click", () => {
  ...
  if (success_counter === 0) {
    alert(
      "Es konnten keine Produkte auf die Einkaufsliste gesetzt werden, da sie sich bereits auf dieser befinden"
    );
  } else {
